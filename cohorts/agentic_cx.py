...
from conversation_agent import DemoConversationAgent
...
with A:
    ...
    def display_enriched_data_download(interaction_data, classified, affinity, campaign):
        """Display download button for enriched JSON with all pipeline data"""
        st.divider()
        st.subheader("📦 Download Enriched Data")
        
        # Build enriched JSON structure
        enriched_data = {
            "pipeline_metadata": {
                "timestamp": pd.Timestamp.now().isoformat(),
                "pipeline_type": "individual_user" if isinstance(interaction_data, dict) else "cohort",
                "data_enriched": True
            },
            "original_interaction": interaction_data if isinstance(interaction_data, dict) else {
                "cohort_id": classified.get("cohort_id"),
                "user_count": len(interaction_data) if isinstance(interaction_data, list) else 1
            },
            "cohort_classification": {
                "cohort_id": classified.get("cohort_id"),
                "cohort_name": classified.get("cohort_name"),
                "intent_level": classified.get("intent_level"),
                "priority": classified.get("priority"),
                "description": classified.get("description"),
                "behavioral_signals": classified.get("behavioral_signals", []),
                "eligibility_rules": classified.get("eligibility_rules", []),
                "exclusion_rules": classified.get("exclusion_rules", []),
                "recommended_channels": classified.get("recommended_channels", []),
                "message_style_tags": classified.get("message_style_tags", []),
                "cooldown_days": classified.get("cooldown_days")
            },
            "affinity_scores": affinity.get("affinity_scores", {}) if affinity else None,
            "affinity_reasoning": affinity.get("llm_reasoning", "") if affinity else None,
            "campaign_ideas": campaign
        }
        
        enriched_json_string = json.dumps(enriched_data, indent=4, default=str)
        
        st.download_button(
            label="📥 Download Enriched JSON (Complete Pipeline Data)",
            data=enriched_json_string,
            file_name=f"enriched_data_{st.session_state.pipeline_identifier}.json",
            mime="application/json",
            use_container_width=True,
            help="Download complete JSON with interaction, cohort classification, affinity scores, and campaigns"
        )
        
        with st.expander("👁️ Preview Enriched JSON Structure"):
    ...
    if pipeline_level == "Individual User":
        ...
        logger.info(f"Users in cluster: \n\n{json.dumps(users_in_cluster, indent=4, default=str)}")
        all_fbc_ids_from_cohort = [u.get("interaction", {}).get("fbc", "user_id",) for u in users_in_cluster]
        selected_user_id = st.selectbox("Select FBC", options=all_fbc_ids_from_cohort)
        selected_user_data = next((u for u in users_in_cluster if u.get("interaction", {}).get("fbc", "user_id",) == selected_user_id), None)
        pipeline_identifier = f"fbc_{selected_user_id}"
    ...
    if run_pipeline:
        ...
        if pipeline_level == "Individual User":
            ...
            display_enriched_data_download(
                interaction_data=selected_user_data["interaction"],
            ...
            with st.spinner("📊 Step 1/3: Loading cohort information..."):
                logger.info(f"Selected User: {json.dumps(selected_user_data, indent=4, default=str)}")
            ...
            #     # logger.info(f"Selected User: {selected_user_data}")
            ...
            with st.spinner("🎯 Step 3/3: Generating cohort campaign ideas..."):
                campaign = CampaignIdeaGeneratorAgent(
                    source=None,
                    classified_cohort=classified,
                    affinity_score= None,  # affinity["affinity_scores"],
                    brochure_url=brochure_url,
                    product_website_url=product_link
                )
                campaign_output = campaign.run(num_of_campaign_ideas=5)
            ...
            display_enriched_data_download(
                        interaction_data=users_in_cluster,
                        classified=st.session_state.classified,
                        affinity=st.session_state.affinity,
                        campaign=st.session_state.campaign
                    )
...
with B:
    ...
    if not st.session_state.get("pipeline_ran", False):
        st.warning("⚠️ Please run the pipeline in the 'Affinity Engine & Customer Profiling' tab first to enable the chatbot.")
        st.stop()
    
    # Get interaction from session state
    interaction = None
    if pipeline_level == "Individual User" and selected_user_data:
        interaction = selected_user_data.get("interaction")
    elif st.session_state.clusters and selected_cohort:
        users_in_cluster = st.session_state.clusters.get(selected_cohort, [])
        if users_in_cluster:
            interaction = users_in_cluster[0].get("interaction")
    
    # Get promotional message
    import random
    random_wa_msg = "Hello! Check out our product."
    if isinstance(st.session_state.campaign, list):
        all_wa_msgs = []
        for camp in st.session_state.campaign:
            for post_set in camp.get('campaign_post_sets', []):
                all_wa_msgs.extend(post_set.get('messages', []))
        if all_wa_msgs:
            random_wa_msg = random.choice(all_wa_msgs)
    
    # Display Promotional Message
    st.info("📢 **Promotional Message Triggered**")
    with st.expander("View Promotional Message", expanded=True):
        st.success(random_wa_msg)
    ...
    if user_input:
        ...
        try:
            d = DemoConversationAgent(
                source=interaction, 
                classified_cohort=st.session_state.classified, 
                affinity_score=st.session_state.affinity, 
                promotional_message=random_wa_msg
            )
            bot_result = d.chat(user_input, history=st.session_state.messages)
