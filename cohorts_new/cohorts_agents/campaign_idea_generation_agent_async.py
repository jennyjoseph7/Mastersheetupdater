...
class CampaignIdeaGeneratorAgent(UtilityMixin):
    ...
    def _len_note(self, max_len: int | None, label: str) -> str:
        if max_len is None:
    ...
    def _output_schema(self) -> str:
        ...
        {{
            "campaign_idea_identifier": <string — exact ID from the batch; {self._len_note(self.title_max_length, 'title')}>,
            "campaign_objective": <string>,
            "campaign_explanation": <string — for media planners; must name product, target audience, insight, channels, featured specs>,
            "audience": [<string>, ...],
            "cta": [
                <string — {self._len_note(self.cta_max_length, 'cta')}>,
                ...
            ],
            "hashtags": [<string>, ...],   // exactly {self.num_of_hashtags}; ≥60% product-specific (include model name)
            "campaign_post_sets": [        // exactly {self.num_of_campaign_post_sets} items
                {{
                    "post_caption": [<string — {self._len_note(self.caption_max_length, 'caption')}>],
                    "hooks":        [<string — {self._len_note(self.hook_max_length, 'hook')}>],
                    "slogan":       [<string — {self._len_note(self.slogan_max_length, 'slogan')}>],
                    "messages":     [<string — {self._len_note(self.message_max_length, 'message')}; 7-8 sentences; product name in first 2; ≥2-3 specific features; subtle emojis; optional closing question>]
                }},
                ...
            ]
        }}
    ...
    def _generate_campaign_ids(self) -> list[str]:
        ...
        - Cover fundamentally different angles (e.g. performance, lifestyle, safety, tech, aspiration, sustainability)
            FORBIDDEN: Generic IDs like "brand_awareness_campaign". No repeated themes.
        - {self._len_note(max_len = self.title_max_length, label = 'title')}
        Return format (strict JSON only):
        {{
            "campaign_idea_ids": ["idea_one", "idea_two"]
        }}
