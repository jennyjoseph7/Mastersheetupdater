...
def do_the_merge(
    ...
    try:
        ...
        png_args = []
        brand_png = get_logo(
            brand, template, files_to_delete, logo_type="Brand", logger=logger
        )
        if brand_png:
            png_args.append(
                (
                    brand_png,
                    template.get("brand_logo_scale", 1.0),
                    template.get("brand_logo_position_x", 0.0),
                    template.get("brand_logo_position_y", 0.0),
                )
            )
        rooftop_png = get_logo(
            rooftop, template, files_to_delete, logo_type="Rooftop", logger=logger
        )
        if rooftop_png:
            png_args.append(
                (
                    rooftop_png,
                    template.get("dealership_logo_scale", 1.0),
                    template.get("dealership_logo_position_x", 0.0),
                    template.get("dealership_logo_position_y", 0.0),
                )
            )
        else:
            dealership_png = get_logo(
                dealership,
                template,
                files_to_delete,
                logo_type="Dealership",
                logger=logger,
            )
            if dealership_png:
                png_args.append(
        ...
        svg_args = []
        dealership_details_svg_path = manage_svg(
            "dealership_details", rooftop, template, files_to_delete
        )
        if dealership_details_svg_path:
            svg_args.append((dealership_details_svg_path, 1.0, 0, 0))
        offer_details_svg_path = manage_svg(
            "offer_details", offer, template, files_to_delete
        )
        if offer_details_svg_path:
            svg_args.append((offer_details_svg_path, 1.0, 0, 0))
        slogan_svg_path = manage_svg(
            "slogan", campaign_details, template, files_to_delete
        )
        if slogan_svg_path:
...
def get_rooftop_type(campaign_type: str):
    return {
        "pre-sales": "showroom",
        "post-sales": "workshop",
        "exchange": "buyback_center",
    }.get(campaign_type)


def manage_hashtags(
    campaign_hashtags: typing.Union[list[str], str], previous_hashtags: str = None
):
    previous_hashtags = previous_hashtags or ""
    campaign_hashtags = (
        re.split(",|\\||;|:| |\n|\r", campaign_hashtags)
        if isinstance(campaign_hashtags, str)
        else campaign_hashtags
    )
...
@gryd.is_a_task(
    function_name="create_campaign_image", job_param="job", logger_param="logger"
)
def create_campaign_image(
    ...
    campaign_hashtags = manage_hashtags(
        brand.get("hashtags") or [], previous_hashtags=campaign_hashtags
    )
    campaign_hashtags = manage_hashtags(
...
def get_rooftop(
    ...
    rooftop_id: str,
    logger: hp.logging.Logger = None,
    dealership_model: AutocrmModel = None,
):
    logger = logger or mlogger
    rooftop_model = get_rooftop_model(rooftop_type, logger=logger)
...
def get_template_by_supported_brands(
    supported_brands: list[str],
    logger: hp.logging.Logger = None,
    brand_model: AutocrmModel = None,
    template_model: AutocrmModel = None,
):
    logger = logger or mlogger
    ...
    for brand_id in supported_brands:
        template = hp.make_single(
            template_model.list(_page_size=1, _as_option=True, brand_id=brand_id),
            force=True,
        )
        if template:
            return template
...
@gryd.is_a_task(
    function_name="create_rooftop_image", job_param="job", logger_param="logger"
)
def create_rooftop_image(
    ...
    logger = logger or mlogger
    if rooftop_id:
        rooftop, dealership, rooftop_model, dealership_model = get_rooftop(
            rooftop_type, rooftop_id, logger=logger
        )
    else:
        if not all([rooftop_address, rooftop_phone_number, logo_url, brand_id]):
            raise ValueError(
                f"Missing required arguments: rooftop_address, rooftop_phone_number, rooftop_email, logo_url, brand_id"
            )
        rooftop = {
    ...
    template_model = AutocrmModel("autosphere_template")
    idea_params = {
        "brand_id": rooftop.get("supported_brands"),
        "_as_option": True,
        "image_url~": None,
    }
    ...
    if not post_ideas:
        raise ValueError(
            f"No ideas with a base image found for supported brands: {rooftop.get('supported_brands')}"
        )
    ...
    campaign_details = {
        ...
        "hashtags": manage_hashtags(
            post_idea.get("hashtags", [])
            + post_idea.get("brand_hashtags", [])
            + post_idea.get("dealership_hashtags", [])
        ),
...
@gryd.is_a_task(
    function_name="create_rooftop_images_batch", job_param="job", logger_param="logger"
)
def create_rooftop_images_batch(
    post_idea_id: str,
    debug: bool = False,
    job: dict = None,
    logger: hp.logging.Logger = None,
    ...
    rooftop_type = {
        ...
        "post-sales": "workshop",
        "exchange": "buyback_center",
    }.get(post_idea.get("campaign_type"))
    if not rooftop_type:
...
@gryd.is_a_task(
    function_name="pad_and_resize_image", job_param="job", logger_param="logger"
)
def pad_and_resize_image_task(
    image_path: str,
    output_dimensions: list = None,
    job: dict = None,
    logger: hp.logging.Logger = None,
):
    logger = logger or mlogger
    return pad_and_resize_image(
        image_path, output_dimensions=output_dimensions, logger=logger
    )
...
@gryd.is_a_task(
    function_name="gemini_image_generation", job_param="job", logger_param="logger"
)
def gemini_image_generation(
    input_image_url, prompt, number_of_images=1, job=None, logger=None, **kwargs
):
    logger = logger or mlogger
    return gemini_image_generation_task(
        input_image_url, prompt, number_of_images, logger=logger, **kwargs
    )


@gryd.is_a_task(
    function_name="openai_image_generation", job_param="job", logger_param="logger"
)
def openai_image_generation(
    ...
    def replace_background_with_gpt(input_image_url, prompt):
        ...
        input_text_token_count = usage.get("input_tokens_details", {}).get(
            "text_tokens", 0
        )
        input_image_token_count = usage.get("input_tokens_details", {}).get(
            "image_tokens", 0
        )
        output_text_token_count = usage.get("output_tokens_details", {}).get(
            "text_tokens", 0
        )
        output_image_token_count = usage.get("output_tokens_details", {}).get(
            "image_tokens", 0
        )
    ...
    return replace_background_with_gpt(input_image_url=input_image_url, prompt=prompt)


def _firefly_get_access_token(
    client_id: str, client_secret: str, logger: hp.logging.Logger
):
    if not client_id or not client_secret:
        raise RuntimeError(
            "FIREFLY_SERVICES_CLIENT_ID / FIREFLY_SERVICES_CLIENT_SECRET not set."
        )
    resp = requests.post(
...
def _firefly_upload_image(
    local_path: str, client_id: str, token: str, logger: hp.logging.Logger
) -> str:
    ...
    logger.info(
        f"Uploading image to Firefly storage: {url} ({mime}, {len(image_bytes)} bytes)"
    )
    resp = requests.post(
        url,
        headers={
...
@gryd.is_a_task(
    function_name="firefly_image_generation", job_param="job", logger_param="logger"
)
    ...
    **kwargs,
    ...
    def replace_background_with_firefly(input_image_url, prompt):
        ...
        try:
            ...
            local_img_path = do_download(
                input_image_url, files_to_delete=files_to_delete
            )

            local_img_path = pad_and_resize_image(
                local_img_path, output_dimensions=[2688, 1536]
            )
            logger.info(f"Input image size: {os.path.getsize(local_img_path)} bytes")
            ...
            if not status_url:
                raise RuntimeError(
                    f"Firefly generate-async missing statusUrl: {submit}"
                )
            ...
            if ADOBE_ANALYTICS_GLOBAL_COMPANY_ID and ADOBE_ANALYTICS_CLIENT_ID:
    ...
    return replace_background_with_firefly(
        input_image_url=input_image_url, prompt=prompt
    )
...
def validate_prompt(
    ...
    car_model: str = None,
    validate_prompt_model: str = None,
    job=None,
    logger=None,
):
    logger = logger or mlogger
...
def optimize_prompt(
    user_input: str, job: dict = None, logger: hp.logging.Logger = None
):
    logger = logger or mlogger
...
def compare_images_func(
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=f".{original_suffix}", delete=False
    ) as f:
        original_temp_file_path = f.name
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=f".{generated_suffix}", delete=False
    ) as f:
        generated_temp_file_path = f.name
    try:
        original_temp_file_path = download_file(
            original_image_url, original_temp_file_path
        )
        generated_temp_file_path = download_file(
            generated_image_url, generated_temp_file_path
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--kwargs", type=str, required=True, help="kwargs to pass to the function"
    )
