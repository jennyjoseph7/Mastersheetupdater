...
export function MapFields({ formData, updateFormData }: MapFieldsProps) {
  ...
  const findBestMatch = useCallback(
    (source: string) => {
      ...
      const exact = systemTargets.find(
        (t) => t.toLowerCase() === s || t.replace(/_/g, "") === cleanS,
      );
      if (exact) return exact;

      if (isPresales) {
        if (cleanS.includes("name")) return "name";
        if (cleanS.includes("mobile") || cleanS.includes("phone"))
          return "phone_number";
        if (cleanS.includes("mail")) return "email";
      } else {
        if (cleanS.includes("reg")) return "reg_number";
        if (cleanS.includes("vin") || cleanS.includes("chassis"))
          return "vin_number";
        if (cleanS.includes("model")) return "vehicle_model_name";
      }

      return (
        systemTargets.find((t) => t.replace(/_/g, "").includes(cleanS)) || ""
      );
    },
  ...
  const updateFieldMapping = (id: string, updates: Partial<FieldMapping>) => {
    if (updates.targetField)
      updates.targetField = cleanString(updates.targetField);
    updateFormData({
      fieldMappings: formData.fieldMappings.map((m) =>
        m.id === id ? { ...m, ...updates } : m,
      ),
    });
  };
      ...
      >
        <div className="flex items-center gap-3">
          <div
            className={cn(
              "p-2 rounded-lg",
              isPresales ? "bg-sky-500/20" : "bg-indigo-500/20",
            )}
          >
            {isPresales ? (
                ...
                <div className="col-span-1 flex justify-center">
                  <Switch
                    ...
                    onCheckedChange={(enabled) =>
                      updateFieldMapping(mapping.id, { enabled })
                    }
                ...
                <div className="col-span-6 flex flex-col gap-1.5">
                  <Popover>
                    ...
                    <PopoverContent className="w-[320px] p-0" align="start">
                      <Command>
                        <CommandInput
                          placeholder={`Search ${isPresales ? "Lead" : "Vehicle"} fields...`}
                          className="h-10"
                        />
