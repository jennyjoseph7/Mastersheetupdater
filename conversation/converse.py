...
from config import AUTOCRM_CONVERSATION_SERVICE_NAME,AUTOCRM_APP_ENTERPRISE_ID, AUTOCRM_CONVERSATION_DEFAULT_LANGUAGE, AUTOCRM_RESPONSE_PROVIDED_UNITS, AUTOCRM_RESPONSE_PROVIDED_PRICE, AUTOCRM_RESPONSE_PROVIDED_UNITS, AUTOCRM_RESPONSE_PROVIDED_ITEM,AUTOCRM_CORE_SERVICE_NAME, AUTOCRM_CAMPAIGN_SERVICE_NAME
...
def converse(*args, **kargs):
    ...
    pass_kwargs["gender"] = kargs.get("gender","male")
    ...
    pass_kwargs["language"] = request_data.get("language")
...
def post_messages_data(*args, **pass_kwargs):
    ...
    with get_pg_connector() as pg:
        ...
        session_model_data = pg.get("session","session_id",pass_kwargs.get("session_id"))
        incoming_message = {
            "reply_to":"",
        ...
        session_data_cache = setup_session_data_cache(session_id=pass_kwargs.get("session_id"),channel=session_model_data.get("channel",""),gender=pass_kwargs.get("gender",""))
...
def setup_session_data_cache(*args, **kwargs):
    ...
    with get_pg_connector() as pg:
        ...
        current_data["preferred_language"] = language or "english"
        mlogger.info("preferred_language == {}".format(language))
