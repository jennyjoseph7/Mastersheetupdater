...
FORWARD_WEBHOOK_URL="https://autobot-webapp-dev.gryd.in/webhook/whatsapp/airtel"
FORWARD_NUMBERS={"9187210943","919187210943","9187210940","919187210940"}
...
# spark/adobe firefly (structure-guided generate-async; OAuth client credentials)
FIREFLY_SERVICES_CLIENT_ID = os.environ.get("FIREFLY_SERVICES_CLIENT_ID")
...
def blacklist_a_number(phone_number, dealership_id = None, region_id = None, channel = None):
    ...
    if not bll:
        return blm.post({
            ...
            "region_id": region_id, 
            "dealership_ids": hp.make_list(dealership_id) if dealership_id else None, 
            "channels": hp.make_list(channel) if channel else None
        })
    cdealerships = bll.get('dealership_ids') or []
    to_update = {}
    if dealership_id and dealership_id not in cdealerships:
        #blm.iadd(phone_number, 'dealership_ids', [dealership_id])
        to_update["dealership_ids"] = cdealerships.append(dealership_id)
    cchannels = bll.get('channels') or []
    if channel and channel not in cchannels:
        #blm.iadd(phone_number, 'channels', [channel])
        to_update["channels"] = cchannels.append(channel)
    if to_update:
...
AUTOCRM_VOICE_INBOUND_SERVICE_NAME_1 = os.environ.get("AUTOCRM_VOICE_INBOUND_SERVICE_NAME_1", "autocrm-voice-1-inbound")
AUTOCRM_VOICE_INBOUND_SERVICE_NAME_2 = os.environ.get("AUTOCRM_VOICE_INBOUND_SERVICE_NAME_2", "autocrm-voice-2-inbound")
AUTOCRM_VOICE_INBOUND_SERVICE_NAME_3 = os.environ.get("AUTOCRM_VOICE_INBOUND_SERVICE_NAME_3", "autocrm-voice-3-inbound")
