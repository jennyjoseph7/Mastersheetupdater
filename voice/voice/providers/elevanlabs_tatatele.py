...
class CallSession:
    ...
    async def outbound_media_stream(self, wb):
        ...
        async def handle_tatatele_media_message(tt_msg: dict):
            ...
            if self.dave_ws and not self.stop_event.is_set():
                try:
                    ...
                    config.flag_error_to_session_or_lead(self.call_id, f"Failed to send to ElevenLabs: {e}")
                    self.dave_ws = None
        ...
        try:
            ...
            async def tatatele_reader(timeout = 60):
                ...
                while not self.stop_event.is_set():
                    try:
                        ...
                        self.stop_event.set()
                        config.flag_error_to_session_or_lead(self.call_id, f"Tatatele reader error: {e}")
                        break
            ...
            async def dave_reader(timeout=60):
                ...
                while not self.stop_event.is_set():
                    try:
                        ...
                        self.stop_event.set()
                        config.flag_error_to_session_or_lead(self.call_id, f"Dave reader error: {e}")
                        break
            ...
            config.flag_error_to_session_or_lead(self.call_id, f"Main error: {e}")
        finally:
    ...
    async def connect_external_websocket(self, url: str, timeout = 32):
        ...
        try:
            ...
            try:
                ...
                config.flag_error_to_session_or_lead(self.call_id, f"Failed to establish WebSocket connection to {url}: {conn_error}")
                raise
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    config.flag_error_to_session_or_lead(self.call_id, f"Recv error: {e}")
                finally:
                    self.stop_event.set()
                    break
        except Exception as e:
            logger.exception(f"[{self.call_id}] connection failed: %s", e)
            config.flag_error_to_session_or_lead(self.call_id, f"Connection failed: {e}")
        finally:
...
def make_call_tatatele(session_data, *args, **kwargs):
    ...
    try:
        ...
        config.flag_error_to_session_or_lead(session_id, f"Connection failed: {exc}")
