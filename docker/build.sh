...
function print_worker_git_info() {
    ...
    commit=$(git log -1 --date=short --pretty=format:"%H|%an|%ad|%s" -- "$WORKER_NAME")

    if [ -n "$commit" ]; then
        IFS="|" read -r sha author date message <<< "$commit"

        mkdir -p "$WORKER_NAME"
        echo "$(git log -1 --pretty=format:"%H - %an, %ad : %s" -- "$WORKER_NAME")" > "$WORKER_NAME/version.sha"

        printf "%-20s %-10s %-18s %-12s %s\n" \
            "$WORKER_NAME" "$sha" "$author" "$date" "$message"
    fi
}
