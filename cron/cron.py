...
def performance_summary(from_time_ms=None):
    ...
    with get_pg_connector() as pg:
        ...
        for campaign_id, campaign_type in campaigns:
            ...
            # Skip invalid or null campaign data
...
#     """
#     Optimized active session manager.
#     """
...
#                             m.dict->>'session_id' = s.dict->>'session_id'

#                             AND

#                             (
#                                 (
#                                     (m.dict->>'created')::DOUBLE PRECISION >
#                                     COALESCE(
#                                         (s.dict->>'history_updated_time')::DOUBLE PRECISION,
#                                         0
#                                     )
#                                 )
...
#                                 (
#                                     (m.dict->>'updated')::DOUBLE PRECISION >
#                                     COALESCE(
#                                         (s.dict->>'history_updated_time')::DOUBLE PRECISION,
#                                         0
#                                     )
...
#                             COALESCE(
#                                 (s.dict->>'last_post_process_time')::DOUBLE PRECISION,
#                                 0
#                             )
...
#                     (
#                         (s.dict->>'last_response_time')::DOUBLE PRECISION
#                         + %s
#                     )
...
#         -- sessions having actual new messages
...
#         -- sessions eligible for post process
...
#                 mlogger.info(f"Processing session_id={session_id} channel={channel}")

#                 needs_history_update = row.get("needs_history_update")

#                 needs_post_process = row.get("needs_post_process")

#                 inactive_cutoff_epoch = row.get("inactive_cutoff_epoch")

#                 last_history_epoch = row.get("last_history_epoch")
...
#                         AND
#                         (
#                             ((dict->>'created')::DOUBLE PRECISION > %s)
#                             OR
#                             ((dict->>'updated')::DOUBLE PRECISION > %s)
#                         )
#                     ORDER BY (dict->>'created')::DOUBLE PRECISION ASC
#                     """

#                     history_rows = pg.fetch_all(message_query, (
#                         session_id,
...
#                         record = message_row.get("dict") or {}

#                         ts = normalize_ts(record.get("created") or record.get("updated"))
...
#                         pg.update("session", "session_id", session_id, update_payload)

#                         mlogger.info(f"Updated history for session_id={session_id} messages_added={len(appended_history)}")
