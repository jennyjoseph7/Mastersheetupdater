	...
	for a in ${START_AGENTS//,/ };do
		...
		jq -c '.agents[]' start_worker_config.json | while IFS= read -r wmap;do
			...
			if [ "$WORKER_NAME" == "$a" ];then
				...
				nohup $worker_path -m agents/$WORKER_ENTRYPOINT -n $WORKER_PARALLEL_THREADS --shutdown-time=$WORKER_SHUTDOWN_TIME 1>> ${LOGDIR}/${WORKER_NAME}_stdout.log 2>> ${LOGDIR}/${WORKER_NAME}_stderr.log &
			fi
		done
	done
	...
	for w in ${START_WORKERS//,/ };do
		...
		jq -c '.workers[]' start_worker_config.json | while IFS= read -r wmap;do
			...
			if [ "$WORKER_NAME" == "$w" ];then
				...
				nohup $worker_path -m $WORKER_NAME/$WORKER_ENTRYPOINT -n $WORKER_PARALLEL_THREADS --shutdown-time=$WORKER_SHUTDOWN_TIME 1>> ${LOGDIR}/${WORKER_NAME}_stdout.log 2>> ${LOGDIR}/${WORKER_NAME}_stderr.log &
			fi
		done
	done
